...
<table class="tarificador">
    ...
    $tablaneumaticos = mysql_query("SELECT * FROM neumaticos WHERE 
    ...
    $contador = 1;
    if (mysql_num_rows($tablaneumaticos) == 0) {
        ?>
        <tr><td colspan="12" style="font-size: 30px;">No se han encontrado coincidencias.</td></tr>
    <?php
    } else {
        while ($linea = mysql_fetch_row($tablaneumaticos)) {
            $contador++;
            if (stripos($linea[5], "MICHELIN") !== FALSE ||
                    stripos($linea[5], "BRIDGESTONE") !== FALSE ||
                    stripos($linea[5], "PIRELLI") !== FALSE ||
                    stripos($linea[5], "DUNLOP") !== FALSE ||
                    stripos($linea[5], "CONTINENTAL") !== FALSE ||
                    stripos($linea[5], "GOOD") !== FALSE) {
                $max = 45;
            }
            $clase = "";
            if ($_GET['dto'] > $max) {
            ...
            <tr <?php echo $clase; ?> >
                ...
                <td class="oferta"><a title="IMPRIMIR" onclick="javascript:imprSelec('<?php echo "bono" . $contador; ?>');" style="cursor:pointer;" >
                        ...
                        $dto = str_replace(",", ".", $dto);
                        $foo = str_replace(",", ".", $linea[8]);
                        $residuo = str_replace(",", ".", $linea[9]);
                        $pvp = number_format(round((((($foo) * $dto) + 12 + $residuo) * 1.21) * $_GET['cantidad'], 2), 2) . "€";
                        echo $pvp;
                        $hoy = new DateTime();
                        ?>
                    </a> 
        ...
        <?php } ?>
                            ...
                            <tr><td colspan="2"><h2>Marca: <?php echo $linea[5]; ?></h2></td></tr>
