...
$result = mysql_query("SELECT * FROM o_proveedores WHERE fecha_recibido NOT LIKE '%000%' ORDER BY fecha_recibido DESC LIMIT ".$cantidad."");
if(isset($_GET['pagina'])){
    $result = mysql_query("SELECT * FROM o_proveedores WHERE fecha_recibido NOT LIKE '%000%' ORDER BY fecha_recibido DESC LIMIT ".$cantidad * ($_GET['pagina']-1).",".$cantidad."");
}
$senpaginas =  mysql_query("SELECT * FROM o_proveedores");
$paginas = mysql_num_rows($senpaginas)/50;
if(isset($_GET['pagina'])){
    $pag=$_GET['pagina'];
}else{
    $pag=1;
