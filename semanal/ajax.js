...
function nuevoPedido() {
    ...
    if (campo.value === "") {
        ...
        alert("Debes introducir la fecha del siguiente semanal");
        prox.style.border = '3px solid red';
        prox.style.background = '#ffcccc';
        mensaje.style.color = 'red';
    } else {
        guardarPedido.open("GET", "npedido.php?fecha=" + campo.value, true);
        guardarPedido.send(null);
        guardarNuevoSemanal.open("GET", "proxSemanal.php?fecha=" + prox.value, true);
        guardarNuevoSemanal.send(null);
        window.location.href = "semanal.php?encurso=SI";
    }
}
