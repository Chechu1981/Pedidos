...
<html>
    <head>
        ...
        function activar() {
            ...
            if (isset($_GET['diasemana']) and $_GET['diasemana'] == 6) {
                $a = false;
            }

            global $mysqli;
            $sen2 = $mysqli->query("SELECT * FROM fiestas;");
            while ($festivo = $sen2->fetch_row()) {
                if ($_GET['numes'] == $festivo[2] and $_GET['dia'] == $festivo[1]) {
                    $a = false;
                }
            }
        ...
        function actualizar() {
            global $mysqli;
            $sen = $mysqli->query("SELECT * FROM lineas WHERE fecha_pedido like '" . $_GET['dia'] . $_GET['mes'] . $_GET['ano'] . "' ORDER BY pedido,destino,cliente;");
            if (@$sen->num_rows > 0) {
                while ($fila = $sen->fetch_row()) {
                    if (isset($_GET['marcado']) and isset($_POST[$fila[0]])) {
                        $mysqli->query("UPDATE  lineas SET pedido='checked=\"checked\"' WHERE id='" . $fila[0] . "'");
                    } elseif (isset($_GET['marcado'])) {
                        $mysqli->query("UPDATE lineas SET pedido='' WHERE id='" . $fila[0] . "'");
                    }
                    if (isset($_GET['marcado']) and isset($_POST['ps' . $fila[0]])) {
                        $mysqli->query("UPDATE lineas SET ps='checked=\"checked\"' WHERE id='" . $fila[0] . "'");
                    } elseif (isset($_GET['marcado'])) {
                        $mysqli->query("UPDATE lineas SET ps='' WHERE id='" . $fila[0] . "'");
                    }
                }
            }
        }
...
<?php $funcionTratarFecha; ?>
            ...
            function verCalendario() {
                if (document.getElementById('calendario').style.display === "none") {
                    ...
                    setTimeout('$("#buscreferencia").focus()', 300);
                } else {
            ...
            $(function() {
                $("#calen").datepicker({
                    ...
                    onSelect: function(datos) {
                        ...
                        window.location.href = "pedidos.php?ano=" + fecha[3] + "&mes=" + fecha[1] + "&numes=" + fecha[2] + "&dia=" + fecha[0] + "&diasemana=" + diasemana;
                    }
                });
            });
        ...
        <div class="contenedor">
                            ...
                            <table>
                                ...
                                <tr>
                                    <td style="text-align: center;">
                                        ...
                                        if ($_SERVER['REMOTE_USER'] != "medina") {
                                            ?>
                                            <a href="./estado.php?dia=<?php echo $_GET['dia']; ?>&mes=<?php echo $_GET['mes']; ?>&ano=<?php echo $_GET['ano']; ?>" target="_blank"  onClick="window.open(this.href, this.target, 'width=400,height=580');
                                                        return false;" ><span class="estadisticas" >Mostrar estadísticas</span></a>
                                           <?php } ?> 
                                    </td>
                                </tr>
                                <tr>
            ...
            <div class="principal">
                ...
                <div class="banda">
                    ...
                    <a href="./prueba.php?dia=<?php echo $_GET['dia']; ?>&mes=<?php echo $_GET['mes']; ?>&ano=<?php echo $_GET['ano']; ?>" target="_blank"  onClick="window.open(this.href, this.target, 'width=700,height=800');
                            return false;" >
...
//                if (@$_GET['diasemana'] == 6 || @$_GET['diasemana'] == 0) {
//                    $activo = false;
//                }
                if ($activo) {
                    ?>
                    ...
                    $prealizado = "<div class='resultado'>Pedido Realizado</div>";
                    if (isset($_GET['diasemana']) and $_GET['diasemana'] > 5) {
                        $prealizado = "";
                    }

                    $sen = $mysqli->query("SELECT * FROM fiestas;");
                    while ($festivo = $sen->fetch_row()) {
                        if ($_GET['numes'] == $festivo[2] and $_GET['dia'] == $festivo[1]) {
                            $prealizado = "";
                        }
                    }
                ...
                if (isset($_POST['referencia']) and $_POST['referencia'] != "") {
                    if ($_SERVER['REMOTE_USER'] == "medina") {
                        $mysqli->query("INSERT INTO lineas 
                ...
                $contador = 1;
                if ($_SERVER['REMOTE_USER'] == "medina") {
                    @$sentencia = $mysqli->query("SELECT * FROM lineas WHERE fecha_pedido LIKE '" . $_GET['dia'] . $_GET['mes'] . $_GET['ano'] . "' AND cliente LIKE '%MEDINA%' ORDER BY pedido,destino,cliente,referencia;");
                } else {
                    @$sentencia = $mysqli->query("SELECT * FROM lineas WHERE fecha_pedido LIKE '" . $_GET['dia'] . $_GET['mes'] . $_GET['ano'] . "' ORDER BY pedido,destino,cliente,referencia;");
                }
                                        ...
                                        if ($activo) {
                                            ?><th style="padding: 0"><!--<button <?php if ($_SERVER['REMOTE_USER'] == 'recepcion') { ?> disabled="disabled" <?php } ?> onclick="comentarios()" title="Guardar comentario" class="boton_comentario" >Matrícula/Comentario</button>-->Matrícula/Comentario</th><th>Cliente/OR</th><th style="padding-left: 45px;padding-right: 45px"> Hora </th><th></th><th>SALIDA</th><th><input<?php if ($_SERVER['REMOTE_USER'] == 'recepcion') { ?> disabled="disabled" <?php } ?> title="Seleccionar todas las lineas" onclick="javascript:seleccionar();" type="checkbox"  class="caja" /></th><th>PS</th><?php
                                                ...
                                                ?><th>Matrícula/Comentario</th><th>Cliente/OR</th><th style="padding-left: 45px;padding-right: 45px"> Hora </th><th></th><th>SALIDA</th><th>PS</th><?php
                                                                    ...
                                                                    if ($_SERVER['REMOTE_USER'] == "medina") {
                                                                        @$sentencia = $mysqli->query("SELECT * FROM lineas WHERE destino LIKE 'M' AND fecha_pedido like '" . $_GET['dia'] . $_GET['mes'] . $_GET['ano'] . "' AND cliente LIKE '%MEDINA%' ORDER BY pedido,destino,cliente,referencia;");
                                                                    } else {
                                                                        @$sentencia = $mysqli->query("SELECT * FROM lineas WHERE destino LIKE 'M' AND fecha_pedido like '" . $_GET['dia'] . $_GET['mes'] . $_GET['ano'] . "' ORDER BY pedido,destino,cliente,referencia;");
                                                                    }
                                                ...
                                                <tr <?php echo $encontrado . $gris . $imp_negr ?> >
                                                    <td class='numero' style='border-right: 1px solid windowtext;'><?php echo $contador++; ?></td>
                                                    <td><?php echo $fila[1]; ?></td>
                                                    <td style='text-align:center' <?php if ($activo && $_SERVER['REMOTE_USER'] != 'recepcion') { ?> class='actualizarCantidad' onclick='abrircantidad(<?php echo $fila[0] . "," . $fila[2]; ?>)' <?php } ?>><?php echo $fila[2] ?></td>
                                                    ...
                                                    if ($activo) {
                                                        if ($_SERVER['REMOTE_USER'] == 'recepcion') {
                                                            ...
                                                            echo "<td><textarea size='50' type='text' onblur='comentarios(" . $fila[0] . ")' title='El comentario se grabará cuando se actualice el pedido.' id='comentario" . $fila[0] . "' name='comentario" . $fila[0] . "' value='" . $fila[4] . "'>" . $fila[4] . "</textarea></td>";
                                                        }
                                                    } else {
                                                        echo "<td>" . $fila[4] . "</td>";
                                                    }
					...
					<td style='font-size:12px;'>" . date_format($date, 'd/') . $mes . date_format($date, '/Y') . ' ' . date_format($date, 'H:i:s') . "</td>";
						...
						<a title='ELIMINAR " . $fila[3] . " " . $fila[1] . "' onClick='javascript:confirmar_linea_pedido(" . $fila[0] . ",\"" . $fila[1] . "\");' target='_blank'>
                                                <img src='../imagenes/eliminar.png' style=\"cursor:pointer;\">
                                                        ...
                                                        ?></td>
                                                        <td style="text-align:center;">
                                                            <input type="checkbox" <?php if ($_SERVER['REMOTE_USER'] == 'recepcion') { ?> disabled="disabled" <?php } ?> name='<?php echo $fila[0]; ?>' <?php echo $fila[9]; ?> />
                                                        </td>
                                                        <td style='text-align:center;'>
                                                            <input type="checkbox" <?php if ($_SERVER['REMOTE_USER'] == 'recepcion') { ?> disabled="disabled" <?php } ?> <?php echo $ps; ?> name='<?php echo "ps" . $fila[0]; ?>' <?php echo $fila[10]; ?> />
                                                        </td>
                                                    </tr>
                                                    <?php
                                                } else {
                                ...
                                <table width='100%;' class='listado'>
                                    ...
                                    if ($activo) {
                                        ?><th></th><th>PS</th><?php
                                                                    ...
                                                                    $numero = 0;
                                                                    //Escribo las lineas en la tabla
                                                                    @$sentencia = $mysqli->query("SELECT * FROM lineas WHERE destino LIKE 'T' AND fecha_pedido like '" . $_GET['dia'] . $_GET['mes'] . $_GET['ano'] . "' ORDER BY pedido,destino,cliente,referencia;");
                                                                    while ($fila = $sentencia->fetch_row()) {
                                                                        $numero++;
                                                                        $imp_negr = "";
                                                                        $encontrado = "";
                                        ...
                                        <tr <?php echo $encontrado . $gris . $imp_negr ?> >
                                            <td class='numero' style='border-right: 1px solid windowtext;'><?php echo $contador++; ?></td>
                                            <td><?php echo $fila[1]; ?></td>
                                            <td <?php if ($activo && $_SERVER['REMOTE_USER'] != 'recepcion') { ?> class="actualizarCantidad" onclick='abrircantidad(<?php echo $fila[0] . "," . $fila[2]; ?>)' <?php } ?>  style='text-align:center'><?php echo $fila[2] ?></td>
                                            <td><?php echo $fila[3]; ?></td>
                                            <?php
                                            if ($activo) {
                                                if ($_SERVER['REMOTE_USER'] == 'recepcion') {
                ...
                echo "<div class='resultado'>" . utf8_encode($texto) . "</div>";
...
<?php include_once '../scripts/pie.php'; ?>
        ...
        $fichero = $mysqli->query("SELECT * FROM lineas WHERE fecha_pedido LIKE '" . $_GET['dia'] . $_GET['mes'] . $_GET['ano'] . "' ORDER BY pedido,destino,cliente,referencia;");
        $ped = fopen($nombre, "w");
        while ($pieza = $fichero->fetch_row()) {
            if ($pieza[8] == "") {
                fputs($ped, $pieza[1]);
                fputs($ped, ";");
                fputs($ped, $pieza[2]);
