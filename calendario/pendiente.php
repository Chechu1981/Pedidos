            ...
            <div class="principal">
                ...
                <div>
                    <table border='2' width='100%;' class='listado'>
                        ...
                        <?php
                        $contador = 1;
                        $sentencia = mysql_query("SELECT * FROM lineas WHERE ps LIKE 'checked=\"checked\"' ORDER BY fecha DESC");
                        if ($destino == "T") {
                            ?><h1>Taller</h1> <?php
                                $sentencia = mysql_query("SELECT * FROM lineas WHERE destino LIKE '" . $destino . "' AND ps LIKE 'checked=\"checked\"' ORDER BY fecha DESC");
                            } elseif ($destino == "M") {
                                ?><h1>Mostrador</h1> <?php
                            $sentencia = mysql_query("SELECT * FROM lineas WHERE destino LIKE '" . $destino . "' AND ps LIKE 'checked=\"checked\"' ORDER BY fecha DESC");
                        } else {
