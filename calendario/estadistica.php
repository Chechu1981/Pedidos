    ...
    <body>
        <div class="contenedor">
            ...
            <div class="banda">
                ...
                $suma = 0;
                $sumames = 0;
                $media = $mysqli->query("SELECT COUNT(*) AS cantidad, fecha_pedido FROM lineas GROUP BY fecha_pedido;");
                $pedidomes = calcular_mes() . calcular_ano();
                $mediames = $mysqli->query("SELECT COUNT(*) AS cantidad, fecha_pedido FROM lineas WHERE fecha_pedido LIKE '%".$pedidomes."%' GROUP BY fecha_pedido;");
                while ($row1 = $media->fetch_row()) {
                    $suma = $suma + $row1[0];
                }
                while ($row2 = $mediames->fetch_row()) {
                    $sumames = $sumames + $row2[0];
                }
                ?>
                <h2 style="padding:15px;">Lineas diarias de VP. </h2>
                <div style="float: right;padding: 8px;">
                    <h4 style="margin: 0px;color: black;">Media: <?php echo round($suma / $media->num_rows, 2); ?></h4>
                    <h4 style="float: right; color: black;">Media de <?php echo calcular_mes() . ': ' . round($sumames / $mediames->num_rows, 2); ?></h4>
                </div>
            </div>
            <div id="cabecera" >
                ...
                <div style="background-color: antiquewhite">
                    <?php
                    $totales = $mysqli->query("SELECT COUNT(id) AS cantidad, fecha_pedido FROM lineas GROUP BY fecha_pedido ORDER BY id DESC LIMIT 100;")  or die("Error en búsqueda.".  mysql_error());
                    while ($row = $totales->fetch_row()) {
