...
<html>
    ...
    <body onload="document.getElementById('cliente').focus();">
        ...
        <div class="contenedor">
            ...
            if (@$_GET['id'] != "") {
                @$sen = $mysqli->query("SELECT * FROM hoja1 WHERE id_contacto = " . $_GET['id'] . ";");
                $fi = $sen->fetch_row();
                ?>
            ...
            //$sentencia=$mysqli->query("SELECT * FROM hoja1 ORDER BY nombre;");
            if (@$_POST['cliente'] === "") {
                @$sentencia = $mysqli->query("SELECT * FROM hoja1 WHERE denominacion LIKE '%" . $_POST['denom'] . "%' ORDER BY nombre;");
            } elseif (@$_POST['denom'] == "") {
                @$sentencia = $mysqli->query("SELECT * FROM hoja1 WHERE Nombre LIKE '%" . $_POST['cliente'] . "%' ORDER BY nombre;");
            } else {
                @$sentencia = $mysqli->query("SELECT * FROM hoja1 WHERE Nombre LIKE '%" . $_POST['cliente'] . "%' AND  denominacion LIKE '%" . $_POST['denom'] . "%' ORDER BY nombre;");
            }
            ?>
            <div id="tabla"></div>
        ...
        <script type="text/javascript">
            ...
            function incluirDestino(cod, clie) {
                var posicion_x = (screen.width / 2) - (150 / 2);
                var posicion_y = (screen.height / 2) - (100 / 2);
                window.open("../reparto/dialogoNuevoCliente.php?cod=" + cod + "&cli=" + clie, this.target, 'width=150,height=300,left=' + posicion_x + ',top=' + posicion_y);
            }
            
            function incluirDestinoEx(cod, clie){
                 var posicion_x = (screen.width / 2) - (150 / 2);
                var posicion_y = (screen.height / 2) - (100 / 2);
                window.open("../reparto/dialogoNuevoClienteEnRuta.php?cod=" + cod + "&cli=" + clie, this.target, 'width=150,height=300,left=' + posicion_x + ',top=' + posicion_y);
            }
        </script>
    </body>
</html>